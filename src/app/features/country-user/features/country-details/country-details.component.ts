...
import { GetCountryQuestionHistoryReponseDto, UserCountryRequstDto } from 'src/app/core/models/CountryHistoryDto';
...
export class CountryDetailsComponent implements OnInit {
  ...
  getCountryPillarDetails(pillarID: number) {
    let request: UserCountryGetPillarInfoRequstDto = {
      userID: this.userService?.userInfo?.userID ?? 0,
      countryID: this.country?.countryID ?? 0,
      pillarID: pillarID,
      updatedAt: this.commonService.getStartOfYearLocal(this.selectedYear)
