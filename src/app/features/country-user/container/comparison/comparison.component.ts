...
export class ComparisonComponent implements OnInit, OnDestroy {
  ...
  getCountryRegion(countryID: number): string {
    return this.countries?.find(c => c.countryID === countryID)?.region || '';
  }
  ...
  exportKpiData() { 
    ...
    this.countryUserService.exportCompareCountries(payload)
      .subscribe({
        next:(res: Blob) => {
