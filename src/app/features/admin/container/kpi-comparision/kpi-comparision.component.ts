...
<<<<<<< HEAD
    if (!this.selectedCountries.length) {
      this.toaster.showWarning("Please select countries to export data.");
      return;
    }
    this.isExportKpi =true;
    let payload: CompareCountryRequestDto = {
      countries: this.selectedCountries
      
    }

    this.adminService.exportCompareCountries(payload)
=======
    ...
    this.adminService.exportCompareCities(payload)
>>>>>>> 9bde2debd31e1f04446351354c9d704a5439b7b1
      .subscribe({
        next:(res: Blob) => {
         this.isExportKpi =false;
        const url = window.URL.createObjectURL(res);
        const a = document.createElement("a");
        a.href = url;
<<<<<<< HEAD
        a.download = "Kpi_Country_Comparison.xlsx";
        a.click();
...
=======
        a.download = "Kpi_City_Comparison.xlsx";
        a.click();
        this.toaster.showSuccess("Report downlaoded successfully");
      },
      error:()=>{
        this.isExportKpi =false;
        this.toaster.showError("There is error accure please try again!");
>>>>>>> 9bde2debd31e1f04446351354c9d704a5439b7b1
      }
    });
  }
